...
changes carried in the two `diff_files` patches — so ckaiser is the confirmed ancestor
...
### 2. Source patches (`diff_files`)

Two real `git diff`s against upstream `231b101`, split by concern and applied in
listed order. Being diffs, they are auditable and make `meson setup` **fail loudly**
if upstream ever drifts (a `git apply` failure aborts configure), instead of silently
masking changes the way a full-file overlay would. They touch disjoint files, so their
relative order is immaterial.

#### `0001-qt6-port.patch` — Qt 6 port

Touches `uglobalhotkeys.h`, `uglobalhotkeys.cpp`, `ukeysequence.cpp`:
...
#### `0002-ukeysequence-operator-bounds-fix.patch` — out-of-bounds bug fix

Touches `ukeysequence.h` only. `UKeySequence::operator[]` guard
`if ((int)n > mKeys.size())` → `if (n >= static_cast<size_t>(mKeys.size()))`.
The upstream `>` is off-by-one: at `n == size()` it fell through and read
`mKeys[size()]` out of bounds (SIGABRT under Qt's debug assertions). This is an
independent correctness fix, not part of the Qt 6 migration, hence its own patch.
Covered by the `UKeySequenceIndex.OutOfRangeReturnsUnknown` regression test in `tests/`.
...
git checkout 231b101` then `git apply .../0001-qt6-port.patch
.../0002-ukeysequence-operator-bounds-fix.patch` and inspect, or diff
